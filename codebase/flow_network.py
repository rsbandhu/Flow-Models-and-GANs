...
class MADE(nn.Module):
	...
	def forward(self, z):
		...
		'''
		#get all the layers in the sequential MADE model
		layers = [l for l in self.net.modules() if type(l) != nn.Sequential]
		...
		outs = self.net(z)
		mu, log_alpha = torch.split(outs, 2, dim=1)
		x = mu + z*torch.exp(log_alpha) # x = mu + z * exp(log(std_dev))
...
class MAF(nn.Module):
	...
	def log_probs(self, x):
		...
		for flow in self.nf:
			...
			#print("\n**** \n")
			#print(log_det.shape)
			log_prob += log_det.view(-1)
		log_prob_base_dist = self.base_dist.log_prob(x).sum(-1)
		#print("***   ", log_prob_base_dist.shape)
		log_prob += log_prob_base_dist
